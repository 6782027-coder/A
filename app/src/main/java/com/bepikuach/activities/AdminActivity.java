...
public class AdminActivity extends AppCompatActivity {
    ...
    private void highlightIconSize(int size, Button s, Button m, Button l) {
        ...
        s.setBackgroundColor(size == 0 ? active : inactive);
        s.setTextColor(size == 0 ? 0xFFFFFFFF : 0xFF3F51B5);
        m.setBackgroundColor(size == 1 ? active : inactive);
        m.setTextColor(size == 1 ? 0xFFFFFFFF : 0xFF3F51B5);
        l.setBackgroundColor(size == 2 ? active : inactive);
        l.setTextColor(size == 2 ? 0xFFFFFFFF : 0xFF3F51B5);
    }
    ...
    private void setupBackgroundMode() {
        ...
        btnBgMode.setOnClickListener(v -> {
            ...
            if (!current) {
                ...
                // בדוק הרשאת Accessibility
                if (!isAccessibilityEnabled()) {
                    new AlertDialog.Builder(this)
                        .setTitle("נדרשת הרשאת נגישות")
                        .setMessage("כדי שהחסימה תפעל ברקע, יש להפעיל את שירות הנגישות של בפיקוח.\n\nלחץ אישור כדי לעבור להגדרות.")
